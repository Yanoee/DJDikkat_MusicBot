...
function buildEmbed(state) {
  ...
  const displayTitle = truncateNowPlayingTitle(title);
  ...
  embed.setDescription(
    [
      '**Now Playing:**',
      uri ? `[${displayTitle}](${uri})` : displayTitle,
    ].join('\n')
  );
  ...
  embed.addFields(
    ...
    { name: '🔁 Loop', value: state.loopCurrent ? 'On' : 'Off', inline: true },
 ...
 */
function buildButtons(guildId, paused, loopCurrent) {
  return [
    new ActionRowBuilder().addComponents(
      ...
      new ButtonBuilder()
        ...
        .setEmoji(loopCurrent ? '🔂' : '🔁')
        .setStyle(loopCurrent ? ButtonStyle.Success : ButtonStyle.Secondary)
    ),
    new ActionRowBuilder().addComponents(
      new ButtonBuilder()
...
async function _upsertController(guildId, state) {
  ...
  const components = buildButtons(guildId, state.paused, state.loopCurrent);
  ...
  if (current) {
    ...
    // Edit failed — send the replacement FIRST, then delete the old card.
    // This order ensures the card never disappears if the send is rate-limited
    // or temporarily fails (e.g. during a long session).
    ...
    if (replacement) {
      ...
      // Send failed — restore the old reference so the next update can retry.
